...
ВАЖНО: твой ответ должен начинаться с { и заканчиваться }. Никакого текста до или после JSON.`
    ...
    const geminiBody = {
      ...
      generationConfig: {
        ...
        // Gemini 2.5 Flash тратит выходные токены на «мышление» и возвращает пустой ответ —
        // из-за этого срабатывал randomFallback («что-то сломалось»). Отключаем мышление
        // и форсим чистый JSON, чтобы ответ всегда доходил целиком.
